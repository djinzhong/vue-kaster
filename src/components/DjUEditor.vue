...
export default {
  ...
  mounted () {
    ...
    this.$once('hook:beforeDestroy', () => {
      let input = document.querySelector('body .uditor-updata')
      if (input) {
  ...
  methods: {
    ...
    updata (editor) {
      ...
      if (!input) {
        ...
        input.classList.add('uditor-updata')
      ...
      input.addEventListener('change', (e) => {
        ...
        axios.post(`${baseUrl}upload.php`, formData).then(res => {
          editor.execCommand('inserthtml', `<img src="${res.data.data.img_url}" />`)
        }).catch(err => {
